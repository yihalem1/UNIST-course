...
def update_trend(selected_country, selected_country_from_dropdown, selected_variable):
    #If a country is selected from the dropdown, prioritize it
    if selected_country_from_dropdown is not None:
        selected_country = selected_country_from_dropdown
    # If selected_country is still None, return an empty figure
    if selected_country is None:
        ...
        fig.update_layout(
            title=f'Trend Over Years for {selected_country} ({selected_variable})',  # Updated title
            xaxis_title='Year',
...
def update_barchart(selected_year):
    filtered_data = data[data['year'] == selected_year]
    top10_countries = filtered_data.sort_values('Score', ascending=False).head(10)
    fig = px.bar(top10_countries, y='Country', x='Score', orientation='h', hover_data=['Score'])
